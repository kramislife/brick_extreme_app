		...
		<div className='flex flex-col p-2 my-2 min-h-screen text-white'>
			<div className='flex flex-col md:flex-row justify-evenly'>
				...
				<div className='flex flex-col-reverse md:flex-row md:w-[50%] max-h-[80vh]'>
					<div className='flex p-2 flex-row md:flex-col w-[25%] relative overflow-y-auto gap-2 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200'>
						{product?.images?.map((img, index) => (
							<img
								className={`w-[100%] object-cover bg-gray-400 border-2 border-gray-800 aspect-square rounded-md cursor-pointer ${
									img.url === activeImage
										? 'border-black'
										: 'border-gray-300'
								} hover:border-black`}
				...
				<div className='opacity-1 flex-1 px-2 '>
					...
					<div className='flex items-center mb-2'>
						<button
							...
							className={`px-4 bg-black py-2 rounded-lg text-lg ${
								quantity === 1
						...
						<span className='text-white px-5 py-2 text-lg bg-black rounded-lg mx-1'>
							{product?.stock > 0 ? quantity : 0}
						</span>
						<button
							onClick={() => handleQuantityChange(1)}
							className={`px-4 py-2 bg-black rounded-lg text-lg ${
					...
					<button
						type='button'
						className='bg-black text-white font-bold py-4 px-7 rounded-lg hover:opacity-70 transition duration-300 w-full md:w-auto mb-2'
