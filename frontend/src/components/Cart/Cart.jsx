...
import { setCartItem, removeCartItem } from '../../redux/features/cartSlice'; // Ensure these actions exist
				...
				<div className='lg:col-span-2 p-5'>
					...
					{cartItems.length === 0 ? (
						<div className='text-center text-lg'>
							Your cart is empty. Please visit the store to add
							products
						</div>
					) : (
						<div className='mb-5'>
				...
				<div className='bg-white p-6 shadow-lg rounded-lg'>
					...
					<button
						className={`w-full px-6 py-3 rounded-lg text-white ${
							cartItems.length === 0
								? 'bg-gray-500 cursor-not-allowed' // Disabled styles
								: 'bg-black hover:opacity-80' // Normal styles
						}`}
						onClick={() => {
							if (cartItems.length > 0) {
								navigate('/shipping'); // Trigger navigation if cart is not empty
							}
						}}
