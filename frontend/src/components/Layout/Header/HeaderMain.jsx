...
import { Link, useLocation, useNavigate } from 'react-router-dom'; // React Router for navigation and routing
import './Header.css'; 
import Search from '../Search/Search'; // Search component for product search functionality
import { useSelector } from 'react-redux'; // Redux hook to access the store
import { useLazyLogoutUserQuery } from '../../../redux/api/authApi'; // Lazy query for logging out the user
import { useGetMeQuery } from '../../../redux/api/userApi'; // Query to fetch current user data
...
const Header = () => {
	...
	return (
		<header
			className={`flex h-[88px] justify-between items-center md:px-4 px-2 transition-all duration-300 ease-in-out relative`}
		>
			...
			>
				<div className='md:w-[300px] flex items-center justify-center'>
					<img
						src={logo}
						alt='Logo'
						className='w-[300px] z-40'
			...
			<div className='flex items-center gap-5'>
				{user ? (
					...
					>
						...
						{/* Dropdown Menu */}
						{isDropdownOpen && (
							<div className='absolute top-10  right-50 mt-2 w-48 bg-white rounded-md shadow-lg z-10'>
								<button
									className='block  w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-200'
									onClick={handleProfileClick}
								>
